...
系统 SHALL 在现有那次句段分析调用中，除语法组合组 `groups[]` 外，还返回一个平行的词级数组 `words[]`。每个词 SHALL 通过请求内本地合并词（`LocalSpans`）的 0 起索引引用一个本地合并词，并返回该词在语境中的单一最佳中文释义、中文语法说明与语境词性（如 自動・カ変、他動・五段、名詞）。词 SHALL NOT 返回 token id、词头或读音——这些由本地分析（jmdic 合并 + UniDic）已提供。词 SHALL 引用请求内已有的本地合并词，不引用本地词典之外的实体。

#### Scenario: Return a meaning for a content word
- **WHEN** 句段含有一个有意义的本地合并词（如 学校）
- **THEN** 响应中该词包含对请求内该合并词的索引、最佳中文释义、语法说明与语境词性，且不含词头/读音

#### Scenario: Omit a particle or auxiliary
- **WHEN** 句段含助词或助动词（如 は、を、です）
- **THEN** 响应可不为其返回词条；悬停时该词回退为仅显示本地词头 + 读音

#### Scenario: Reference a local merged span
- **WHEN** 一个本地合并词横跨多个 UniDic token（如 `で`+`も` 合并为 `でも`）
- **THEN** 该词引用该合并词（一个 span 索引），而非逐个 token，也不重复 token 元数据

### Requirement: Bound and validate word output
系统 SHALL 在接受前校验每个词：引用索引为请求内有效本地合并词、同一合并词不得被重复引用、释义/语法/词性的字段长度受限。无效词 SHALL 被单独丢弃，不影响其余有效词、语法组或本地结果。每句段词级输出 SHALL 有宽松的上限，超出时保留前 N 个并记录诊断。

#### Scenario: Drop a word with an invalid span reference
- **WHEN** 一个词引用了不存在的本地合并词索引
- **THEN** 该词被丢弃，其他有效词继续可用

#### Scenario: Drop a duplicate span reference
- **WHEN** 两个词引用了同一个本地合并词
